...
GITHUB_PATH="\""https://github.com/jameel-yahoo/meta-murata-wireless.git"\""
META_MURATA_WIRELESS_GIT_JAMEEL_YAHOO="https://github.com/jameel-yahoo/meta-murata-wireless.git"
...
if [ -d "$TEST_DIR_NAME" ]; then
	...
	git clone $META_MURATA_WIRELESS_GIT_JAMEEL_YAHOO --quiet
	cd $TEST_DIR_NAME/cyw-script-utils/latest
fi
...
#TO-BE-REMOVED BY JAMEEL
		rm -rf meta-murata-wireless

		git clone $META_MURATA_WIRELESS_GIT_JAMEEL_YAHOO
...
#FMAC_VERSION="1"
	if [ "$FMAC_VERSION" = "1" ] && [ "$BRANCH_TAG_OPTION" = "y" ]; then
		...
		echo "|     |      Release     | branch  | Supported     |     Release Tag         |"
		echo "|-----|------------------|---------|---------------|-------------------------|"
		echo "|  1  | 4.9.88_2.0.0 GA  | rocko   | 6,7,8         | imx-rocko-battra_r1.0   |"  
		echo "|  2  | 4.9.51 8MQ Beta  | morty   | 8             | imx8-morty-battra_r1.0  |"
		echo "|  3  | 4.9.11_1.0.0 GA  | morty   | 6,7           | imx-morty-battra_r1.0   |"
		echo "|  4  | 4.1.15_2.0.0 GA  | krogoth | 6,7 (No 7ULP) | imx-krogoth-battra_r1.0 |"
		echo "------------------------------------------------------------------------------"
		break
		...
		echo "|     |      Release     | branch  | Supported     |     Developer Tag     |"
		echo "|-----|------------------|---------|---------------|-----------------------|"
		echo "|  1  | 4.9.88_2.0.0 GA  | rocko   | 6,7,8         | imx-rocko-battra      |"  
		echo "|  2  | 4.9.51 8MQ Beta  | morty   | 8             | imx8-morty-battra     |"
		echo "|  3  | 4.9.11_1.0.0 GA  | morty   | 6,7           | imx-morty-battra      |"
		echo "|  4  | 4.1.15_2.0.0 GA  | krogoth | 6,7 (No 7ULP) | imx-krogoth-battra    |" 
		echo "----------------------------------------------------------------------------"
		break
	else
		echo -e "${RED}Error: That is not a valid choice, try again.${NC}"
...
iMXrockobattraStableReleaseTag="imx-rocko-battra_r1.0"
...
iMXrockobattraDeveloperRelease="imx-rocko-battra"
...
if [ "$FMAC_VERSION" = "1" ]; then
	...
	if [ "$BRANCH_TAG_OPTION"    = "y" ] && [ "$ENTRY" = "1" ]; then
		BRANCH_RELEASE_OPTION=1
		BRANCH_RELEASE_NAME="$iMXrockobattraStableReleaseTag"
	...
	# rocko-battra
		...
		BRANCH_RELEASE_OPTION=5
		BRANCH_RELEASE_NAME="$iMXrockobattraDeveloperRelease"
		iMXYoctoRelease="$imxrockoYocto"
		YoctoBranch="rocko"
		fmacversion="$BATTRA_FMAC"
...
#DEBUGGGGGG..
echo "BRANCH_RELEASE_OPTION: $BRANCH_RELEASE_OPTION "

while true; do
case $BRANCH_RELEASE_OPTION in
        # For Branch/Release :---------------imx-rocko-battra----------------------
	1|5)
	# For Branch/Tag : ---------------------imx-rocko-battra / r_1.1-------------------------------
...
#       Prompting the user to select TARGET
	...
	case $TARGET_OPTION in
		...
		LINUX_SRC=linux-imx_4.9.88.bbappend@imx8
